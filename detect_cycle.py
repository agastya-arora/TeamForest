def detect_cycle(A): #A is an adjacency list (dictionary in python)
    ...
    def DFS(vertex,parent): #parent is the vertex that "vertex" came from
        visited[vertex]=True
        for neighbor in A[vertex]:
            if visited[neighbor]==False:
                if DFS(neighbor,vertex): #recursive step
                    return True
            else:
                if neighbor!=parent:
                    return True
    ...
    for i in range(NumOfVertices):
        if visited[i]==False:
            if DFS(i,parent=None): #cause initial vertex does not have parent
                return True
    return False
